...
results = train_model(model, criterion, device, trainloader,
                      testloader, optimizer, scheduler, EPOCHS)
...
print(
    f"----------Correctly classified index length is {len(hit_index)}----------")
...
show_images_pred(test_images[miss_index], test_targets[miss_index],
                 test_predictions[miss_index], denorm)
...
show_images_cam(test_images, test_targets, test_predictions,
                heatmaps, miss_index, denorm)


print(f"----------Visualize correctly classified GRADCAM----------")
show_images_cam(test_images, test_targets, test_predictions,
                heatmaps, hit_index, denorm)
